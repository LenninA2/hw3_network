...
void find_checksum_failure(uint8_t *udp_packet, uint32_t len) {
  // flip bits in a single data byte to find the checksum failure
  for (uint32_t i = 9; i < len; i++) {
    for (uint32_t j = 0; j < 8; j++) {
      udp_packet[i] ^= (1 << j); // single bit flip
      uint16_t checksum = compute_checksum(udp_packet, len);
      // failure mode found
      if (checksum == ((udp_packet[6] << 8) | udp_packet[7])) {
        printf("Checksum failure found at byte %d, bit %d\n", i, j);
        return;
      }

      // undo the bit flip (since it didn't result in a failure)
      udp_packet[i] ^= (1 << j);
    }
  }
}
...
int main(void) {
  ...
  // Here, we find a bit flip that causes the checksum
  // to incorrectly match the checksum in the packet!
